...
assert.equal(momentumCards.length, 30);
...
assert.ok(momentumById.get("cm-002-guard-on-the-return").tags.includes("circumstance-neutralization"));
assert.ok(momentumById.get("cm-003-next-in-line").tags.includes("conditional-consumption"));
assert.ok(momentumById.get("cm-003-next-in-line").fallbackDescription.includes("end of your next turn"));
assert.equal(momentumById.get("cm-004-longer-stride").weight, 2);
assert.ok(momentumById.get("cm-004-longer-stride").fallbackDescription.includes("end of your next turn"));
assert.equal(momentumById.get("cm-005-rhythm-carries").impact, "strong");
assert.ok(momentumById.get("cm-005-rhythm-carries").fallbackDescription.includes("reduce that penalty by 2"));
assert.ok(momentumById.get("cm-005-rhythm-carries").tags.includes("conditional-consumption"));
assert.ok(momentumById.get("cm-006-no-time-to-answer").tags.includes("reaction-denial"));
assert.ok(momentumById.get("cm-006-no-time-to-answer").fallbackDescription.includes("could trigger a reaction"));
...
assert.ok(momentumById.get("cm-011-pivot-point").tags.includes("special-movement"));
assert.equal(momentumById.get("cm-012-hands-still-moving").impact, "moderate");
assert.ok(momentumById.get("cm-012-hands-still-moving").tags.includes("free-hand"));
assert.ok(momentumById.get("cm-012-hands-still-moving").tags.includes("reposition"));
assert.ok(momentumById.get("cm-013-hard-to-ignore").tags.includes("mark"));
assert.ok(momentumById.get("cm-014-fear-can-wait").tags.includes("frightened"));
assert.ok(momentumById.get("cm-014-fear-can-wait").fallbackDescription.includes("end of your next turn"));
assert.equal(momentumById.get("cm-015-cut-the-corner").impact, "light");
assert.ok(momentumById.get("cm-015-cut-the-corner").tags.includes("difficult-terrain"));
assert.ok(momentumById.get("cm-015-cut-the-corner").fallbackDescription.includes("end of your next turn"));
assert.ok(momentumById.get("cm-016-center-of-the-fight").tags.includes("space-control"));
assert.ok(momentumById.get("cm-016-center-of-the-fight").tags.includes("difficult-terrain"));
assert.ok(momentumById.get("cm-017-let-them-swing").tags.includes("target-miss"));
assert.equal(momentumById.get("cm-018-adrenaline-dividend").impact, "strong");
assert.ok(momentumById.get("cm-018-adrenaline-dividend").tags.includes("fortune"));
assert.ok(!momentumById.get("cm-018-adrenaline-dividend").tags.includes("temporary-hit-points"));
assert.ok(momentumById.get("cm-019-through-the-gap").tags.includes("tumble-through"));
assert.ok(momentumById.get("cm-019-through-the-gap").fallbackDescription.includes("when the action begins"));
assert.equal(momentumById.get("cm-020-long-follow-through").impact, "strong");
assert.ok(momentumById.get("cm-020-long-follow-through").tags.includes("reach"));
assert.equal(momentumById.get("cm-021-break-contact").impact, "strong");
assert.ok(momentumById.get("cm-021-break-contact").tags.includes("retreat"));
assert.ok(momentumById.get("cm-021-break-contact").tags.includes("reaction-protection"));
assert.ok(momentumById.get("cm-022-living-bulwark").tags.includes("ranged-defense"));
assert.ok(momentumById.get("cm-022-living-bulwark").fallbackDescription.includes("+1 circumstance bonus"));
assert.equal(momentumById.get("cm-023-form-finds-you").impact, "strong");
assert.ok(momentumById.get("cm-023-form-finds-you").tags.includes("stance"));
assert.ok(momentumById.get("cm-023-form-finds-you").fallbackDescription.includes("all normal stance rules"));
assert.ok(momentumById.get("cm-024-momentum-to-spare").tags.includes("reaction-economy"));
assert.ok(momentumById.get("cm-024-momentum-to-spare").tags.includes("frequency-limits"));
assert.ok(momentumById.get("cm-025-break-the-bind").tags.includes("escape"));
assert.ok(momentumById.get("cm-025-break-the-bind").tags.includes("fallback-step"));
assert.ok(!momentumById.get("cm-025-break-the-bind").fallbackDescription.includes("restrained"));
assert.equal(momentumById.get("cm-026-close-quarters-clarity").impact, "strong");
assert.ok(momentumById.get("cm-026-close-quarters-clarity").tags.includes("reaction-window"));
assert.ok(momentumById.get("cm-026-close-quarters-clarity").tags.includes("step"));
assert.ok(momentumById.get("cm-027-turn-the-tables").tags.includes("counter-maneuver"));
assert.ok(momentumById.get("cm-028-dont-blink").tags.includes("sneak"));
assert.ok(momentumById.get("cm-029-victory-echo").tags.includes("defeat-trigger"));
assert.equal(momentumById.get("cm-030-momentum-unspent").impact, "strong");
assert.ok(momentumById.get("cm-030-momentum-unspent").tags.includes("anti-chain"));
assert.ok(momentumById.get("cm-030-momentum-unspent").tags.includes("out-of-turn"));
assert.ok(momentumById.get("cm-030-momentum-unspent").fallbackDescription.includes("-5 multiple attack penalty"));

assert.equal(automated, 4);
assert.equal(manual, 116);
...
console.log("PF2E Critical Forge: Martial Consequences 0.4.3 structural validation passed.");
